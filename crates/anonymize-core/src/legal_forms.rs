...
fn allows_soft_wrap_continuation(
  ...
  comma_before_soft_wrap(text, newline_start, suffix_is_the_continuation)
    || all_caps_name_soft_wrap(text, newline_start, data)
}
...
fn all_caps_name_soft_wrap(
  ...
  is_all_caps_name_token(token)
    && prefix.chars().any(char::is_lowercase)
    && !data
      .structural_single_cap_prefixes
      .contains(lowercase_lookup(structural_prefix).as_ref())
}
...
fn has_disallowed_line_break(
    ...
    .and_then(|tail| tail.find('\n'))
  {
    ...
    let all_caps_name_continuation =
      all_caps_name_soft_wrap(full_text, absolute_newline, data);
...
mod tests {
  ...
  fn org_texts_for_suffix(text: &str, suffix: &str) -> Vec<String> {
    let data = PreparedLegalFormData::new(LegalFormData {
      ...
      structural_single_cap_prefixes: english_vocabulary(include_str!(
        "../../../packages/data/config/structural-single-cap-prefixes.json"
      )),
      ..LegalFormData::default()
    });
    let suffix_start = text.find(suffix).expect("suffix present");
