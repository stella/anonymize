...
fn object_identity_matches(
  ...
  IDENTITY_KEYS.iter().any(|keys| {
    keys.iter().all(|key| {
      expected
        .get(*key)
        .zip(actual.get(*key))
        .is_some_and(|(expected, actual)| equivalent(expected, actual))
    })
  })
}

fn sequence_item_matches(expected: &Value, actual: &Value) -> bool {
  if compatible_shape(expected, actual) {
    return true;
  }
  match (expected, actual) {
    (Value::Object(expected), Value::Object(actual)) => {
      object_identity_matches(expected, actual)
    }
    _ => false,
  }
}
...
fn merge_array(expected: &mut Vec<Value>, actual: &[Value]) {
  let previous = std::mem::take(expected);
  let templates = previous.clone();
  let mut merged = Vec::with_capacity(actual.len());
  let mut expected_index = 0usize;
  let mut actual_index = 0usize;

  while expected_index < previous.len() && actual_index < actual.len() {
    if sequence_item_matches(&previous[expected_index], &actual[actual_index]) {
      let mut value = previous[expected_index].clone();
      merge_current(&mut value, &actual[actual_index]);
      merged.push(value);
      expected_index += 1;
      actual_index += 1;
      continue;
    ...
    let actual_skip = actual[actual_index + 1..]
      .iter()
      .position(|candidate| {
        sequence_item_matches(&previous[expected_index], candidate)
      })
      .map(|offset| offset + 1);
    let expected_skip = previous[expected_index + 1..]
      .iter()
      .position(|candidate| {
        sequence_item_matches(candidate, &actual[actual_index])
      })
      .map(|offset| offset + 1);

    match (actual_skip, expected_skip) {
      (Some(insertions), None) => {
        for value in &actual[actual_index..actual_index + insertions] {
          merged.push(closest_template(value, &templates).map_or_else(
            || normalized_value(value),
            |template| shaped_value(value, template),
          ));
        }
        actual_index += insertions;
      }
      ...
      (None | Some(_), Some(deletions)) => {
        expected_index += deletions;
      }
      (None, None) => {
        let mut value = previous[expected_index].clone();
        merge_current(&mut value, &actual[actual_index]);
  ...
  for value in &actual[actual_index..] {
    merged.push(closest_template(value, &templates).map_or_else(
      || normalized_value(value),
      |template| shaped_value(value, template),
    ));
  ...
  match (expected, actual) {
    (Value::Object(expected), Value::Object(actual)) => {
      ...
      for (key, actual_value) in actual {
        ...
        } else if !is_default_value(actual_value) {
...
mod tests {
  ...
  fn merge_inserts_top_level_and_nested_keys_and_removes_stale_keys() {
    ...
    assert_eq!(expected, json!({ "existing": { "old": 2 } }));
    assert_eq!(object_keys(&expected), ["existing"]);
    assert_eq!(
  ...
  #[test]
  fn merge_aligns_middle_insertion_without_reshaping_heterogeneous_tail() {
    let mut expected = json!([
      { "kind": "row", "id": "a", "value": 1 },
      { "kind": "row", "id": "c", "value": 3 },
      { "kind": "footer", "name": "tail", "enabled": true }
    ]);
    let actual = json!([
      { "kind": "row", "id": "a", "value": 1, "added": false },
      { "kind": "row", "id": "b", "value": 2, "novel": { "flag": true } },
      { "kind": "row", "id": "c", "value": 3, "defaulted": null },
      { "kind": "footer", "name": "tail", "enabled": true, "items": [] }
    ]);
  ...
  #[test]
  fn merge_aligns_multiple_consecutive_insertions() {
    let mut expected = json!([
      { "id": "a", "value": 1 },
      { "id": "d", "value": 4 }
    ]);
  ...
  #[test]
  fn merge_aligns_array_deletion() {
    let mut expected = json!([
      { "kind": "row", "id": "a" },
      { "kind": "row", "id": "removed" },
      { "kind": "row", "id": "c" }
  ...
  #[test]
  fn merge_updates_changed_existing_item_and_is_idempotent() {
    let mut expected = json!([
      { "id": "a", "value": 1 },
      { "id": "b", "value": 2 },
      { "id": "c", "value": 3 }
