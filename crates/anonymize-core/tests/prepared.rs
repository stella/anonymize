...
#[test]
#[ignore = "release-mode scaling regression check"]
fn address_seed_postal_collection_scales_with_input_size() {
  let prepared = PreparedEngine::new(prepared_config! {
    literal_patterns: vec![
      SearchPattern::LiteralWithOptions {
        pattern: String::from("Boston"),
  ...
  for target_bytes in sizes {
    ...
    samples.push((text.len(), best_postal_us, best_total));
  }

  let (smallest_bytes, smallest_postal_us, _) = samples[0];
  let (largest_bytes, largest_postal_us, largest_total) = samples[4];
  assert!(
