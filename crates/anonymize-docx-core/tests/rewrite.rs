...
-> Result<(), Box<dyn std::error::Error>> {
  let source = docx(&format!(
    "<w:document xmlns:w=\"{WORD_NAMESPACE}\"><w:body><w:p><w:r><w:t>😀 [PERSON_session%5Fone_1] [PERSON_other_1]</w:t></w:r></w:p></w:body></w:document>"
  ))?;
  ...
  let candidate = plan
    .blocks
    .first()
    .and_then(|block| block.candidates.first())
