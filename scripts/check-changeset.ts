...
const RUNTIME_SOURCE_RE =
  /^(?:packages\/(?:anonymize|cli|document-docx|document-pdf|mcp)\/(?:(?:src|scripts)\/|(?:index\.cjs|tsconfig(?:\.wasm)?\.json|tsdown\.config\.ts)$)|crates\/anonymize-(?:adapter-contract|core|docx-core|napi|pdf-core|py)\/)/;
...
const RUNTIME_CHANGELOG_RE =
  /^packages\/(?:anonymize(?:-(?:darwin-(?:arm64|x64)|linux-(?:arm64|x64)-gnu|win32-x64-msvc))?|cli|document-docx|document-pdf|mcp)(?:\/wasm)?\/CHANGELOG\.md$/;
