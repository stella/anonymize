...
function describeNativeTimingScenario({
  ...
  return {
    ...
    setupBeforeClickMs: roundMs(
      nativePackageReadMs + nativePrepareMs + nativeWarmPrepareMs,
    ),
    preloadedClickMs: coldRunMs,
    firstTouchMs: roundMs(
      nativePackageReadMs + nativePrepareMs + nativeWarmPrepareMs + coldRunMs,
    ),
...
function collectNativeDiagnostics({ runner, fixtures }) {
  ...
  return {
    ...
    warm:
      runner.warmPrepareDiagnostics === null
        ? null
        : {
            stages: diagnosticStageSummaries(
              runner.warmPrepareDiagnostics.events,
            ),
            topStages: topDiagnosticStages(
              diagnosticStageSummaries(runner.warmPrepareDiagnostics.events),
            ),
          },
    cachedPrepare:
...
function createNativeStaticRunnerFromJson(configJson, stringifyMs = 0) {
  ...
  const warmPrepare = warmNativePreparedSearch(prepared);
  const warmPrepareMs = warmPrepare.ms;
  ...
  for (let index = 0; index < CACHED_PREPARE_ITERATIONS; index += 1) {
    ...
    cachedWarmPrepareMsByIteration.push(
      warmNativePreparedSearch(cachedPrepared).ms,
    );
  ...
  return {
    prepared,
    prepareDiagnostics,
    warmPrepareDiagnostics: warmPrepare.diagnostics,
