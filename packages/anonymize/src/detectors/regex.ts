...
const PII_PATTERNS: readonly PiiPattern[] = [
  ...
  // Domestic CZ/SK phone: 9 digits, starting with 2xx–7xx
  // (landline 2–5, mobile 6–7). Allows spaces/dashes
  // between groups. Negative lookahead excludes bank
  // account segments (digits followed by /bank_code).
  {
    label: "phone number",
    pattern:
      /\b[2-7]\d{2}[\s.-]?\d{3}[\s.-]?\d{3}(?![\s.-]?\d*\/)\b/g,
    score: 0.9,
  },
  {
