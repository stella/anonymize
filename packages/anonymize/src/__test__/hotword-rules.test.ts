...
describe("hotword rules", () => {
  ...
  it("reclassifies date to 'date of birth' " + "near 'narozen'", () => {
    const text = "narozen dne 12.03.1990 v Praze";
  ...
  it("leaves entity unchanged with no nearby hotwords", () => {
    ...
    const entity = makeEntity(93, 103, "date", 0.65);
    const result = applyHotwordRules([entity], text);
    expect(result).toHaveLength(1);
  ...
  it("applies distance decay: closer hotword gives stronger boost", () => {
    ...
    const farText = "tel. " + " ".repeat(50) + "123456789";

    const closeEntity = makeEntity(5, 14, "phone number", 0.5);
    const farEntity = makeEntity(55, 64, "phone number", 0.5);

    const closeResult = applyHotwordRules([closeEntity], closeText);
    const farResult = applyHotwordRules([farEntity], farText);
  ...
  it("keeps strongest adjustment from multiple matching rules", () => {
    ...
    const text = "narozen článek dne 12.03.1990";
  ...
  it("matches hotword before entity within proximityBefore", () => {
    ...
    const entity = makeEntity(76, 83, "address", 0.5);
    const result = applyHotwordRules([entity], text);
    expect(result).toHaveLength(1);
    expect(result[0].score).toBeGreaterThan(0.5);
  });
