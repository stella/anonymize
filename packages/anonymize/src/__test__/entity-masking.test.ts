...
describe("maskDetectedSpans", () => {
  ...
  test("single entity replaced with [MASKED]", () => {
    ...
    expect(result.maskedText).toBe("Hello [MASKED], welcome");
  });
  ...
  test("multiple entities: offsets accumulate", () => {
    ...
    //  j=18, z=29, ' '=30
    const text = "Call Jan Novák at jan@novak.cz today";
    ...
    expect(result.maskedText).toBe("Call [MASKED] at [MASKED] today");
  });
  ...
  test("adjacent entities: no gap issues", () => {
    ...
    const e2 = entity(3, 6);
    const result = maskDetectedSpans(text, [e1, e2]);
    expect(result.maskedText).toBe("[MASKED][MASKED]");
  });
});
...
describe("offsetMap", () => {
  test("NER entity after masked region maps correctly", () => {
    ...
    const text = "Hello Ing. Jan Novák, call 123456";
    const e = entity(6, 20);
    const result = maskDetectedSpans(text, [e]);
    // masked: "Hello [MASKED], call 123456"
    // [MASKED] is 8 chars, shift = 14 - 8 = 6
    expect(result.maskedText).toBe("Hello [MASKED], call 123456");
...
describe("unmaskNerEntities", () => {
  test("maps NER entities back to original offsets", () => {
    ...
    const text = "Ing. Jan Novák lives at Praha 1";
    ...
    const result = unmaskNerEntities(rawNer, mask, text);
  ...
  test("empty NER entities returns empty", () => {
    ...
    const mask = maskDetectedSpans(text, ruleEnts);
    const result = unmaskNerEntities([], mask, text);
    expect(result).toHaveLength(0);
  });
});
