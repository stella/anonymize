...
const PASSPORT_IDENTIFIER_RE =
  /\b(?:(?=[A-Za-z0-9]{0,8}\d)[A-Za-z0-9]{9}|(?:[A-Za-z]{1,2}(?:[\p{Zs}\t]{1,8}|[\p{Zs}\t]{0,8}[-‐‑‒–—―/][\p{Zs}\t]{0,8}|[.])?|[A-Za-z](?:[\p{Zs}\t]{1,8}[.][\p{Zs}\t]{0,8}|[\p{Zs}\t]{0,8}[.][\p{Zs}\t]{1,8}))\d{6,8}|\d{2}[A-Za-z]{2}\d{5}|\d{7,9})\b/u;
const PASSPORT_SEPARATOR_RE = /[\s\-‐‑‒–—―/.]/g;
...
const normalizePassportText = (text: string): string => {
  ...
  return passportIdentifier.replace(PASSPORT_SEPARATOR_RE, "").toUpperCase();
};
